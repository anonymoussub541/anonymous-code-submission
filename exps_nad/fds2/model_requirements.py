...
model_requirements = {
    ...
    "data_background": "label_num is 100. the pixel_values are normalized.",
    "init_parameters": {"label_num": {"type": "int"}, "base_dim": {"type": "int"}, "model_depth": {"type": "int"}},
    "methods": {
        "forward": {
            "inputs": {
                "pixel_values": {"shape": "(batch_size, 3, 32, 32)", "type": "torch.FloatTensor"},
    ...
    "other_requirements": ["The `base_dim` is typically set to 32 and defines a base dim of the model. Specific modules adjust this dimension by expanding or reducing it as needed. (base_dim*2 = 64, base_dim*4=128, base_dim*8=256, ...)",
                          "The `model_depth` defines the number of main layers or blocks in the model at a macro level — typically values like 15 or 18. To create layers or stages, adapt them based on `model_depth`. E.g., use a safe splitting function or define stage_depth = ratio × model_depth.",
                          "Sample size is about 50k, not large. Be cautious about overfitting and early saturation."]
}
