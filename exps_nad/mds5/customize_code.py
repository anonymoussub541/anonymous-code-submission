...
data_flag = 'synapsemnist3d'  # change dataset here
...
def evaluate(model: nn.Module, loader: DataLoader, device=None):
    ...
    label_result = []
    for x in loader:
        output = model.predict(**{k:v.to(device) for k,v in x.items() if k != 'label'})
        pred_result.append(output["logits"].cpu())
        label_result.append(x["label"])
    pred00 = torch.concat(pred_result)
    label00 = torch.concat(label_result)
    loss = F.binary_cross_entropy_with_logits(pred00, label00, reduction='mean').item()
    pred00 = pred00.numpy()
    label00 = label00.numpy()
    rocauc = roc_auc_score(label00, pred00)        
    pred_label = [1 if p > 0.0 else 0 for p in pred00]
    accuracy = accuracy_score(label00, pred_label)
    final_dict = {"eval_loss": round(loss, 6),
                  "eval_auc": round(rocauc, 6), 
                 'eval_acc': round(accuracy, 6)}
    return final_dict
...
EXPECT_METRIC_POINT = 0.9
LIMIT_METRIC = 1.0
key_metrics_name = "eval_auc"
...
init_paras_used ={'base_dim': 32, 'model_depth': 12}
print("init_paras_used: ", init_paras_used)


task_specific_instruction = "Note: no pretrain, no finetune, model will be trained from scratch. Must use model_depth to allocate main blocks/layers depth. Model parameters should be < 1M. " 
